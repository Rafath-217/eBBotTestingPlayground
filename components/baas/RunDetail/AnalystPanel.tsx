...
export default function AnalystPanel({ run }: AnalystPanelProps) {
  ...
  // Handle error case (e.g. no access to revenue data)
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                This store has not granted access to revenue and order data. The analyst requires the <code className="text-xs bg-amber-100 dark:bg-amber-900/50 px-1 py-0.5 rounded">read_reports</code> access scope.
              </p>
              {data.summary && (
                <p className="text-xs text-amber-600 dark:text-amber-500 mt-3 italic">{data.summary}</p>
              )}
            </div>
          </div>
        </div>
