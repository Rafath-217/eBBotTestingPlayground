  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 text-slate-800 dark:text-slate-200 leading-relaxed">
      ...
      <blockquote className="border-l-4 border-slate-300 dark:border-slate-600 pl-4 my-4 text-slate-500 dark:text-slate-400 italic">
        ...
        <p><strong>Related docs:</strong> <code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">PM_FAQ_PIPELINE.md</code> (companion Q&amp;A), <code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">README.md</code> (engineering reference)</p>
      </blockquote>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          {([
            ...
            ['Hint', 'A keyword or label returned by the Structure AI that helps the system match collections or products to the correct step (e.g., the keyword "chocolate" to match "Milk Chocolate" and "Dark Chocolate" collections to a "Pick Chocolates" step). Collection hints use case-insensitive substring matching (e.g., hint "meal" matches "Hot Meals"). Product type hints use exact string matching.'],
            ['Product group hint', 'A more specific hint where the AI groups individual product titles under named categories, used when product types alone are not distinctive enough. Product title matching is case-insensitive exact (e.g., "Blue Hoodie" matches "blue hoodie" but not "Blue Hoodie XL"). Maximum 10 groups per AI response. Hallucinated titles (titles that don\'t exist in the store) are silently filtered out and logged.'],
            ['Hint discard', 'When the AI invents product titles that do not exist in the store\'s actual inventory, those titles are removed. The discarded titles are logged in the decision trace for debugging.'],
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          {([
            ...
            ['Max output tokens', '1024 (Structure, Rules), 2048 (Discount)', 'Maximum length of each AI response'],
            ...
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">{setting}</td>
      ...
      <p className="mb-6">Throughout this document, we use two examples that exercise different paths through the system:</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          <tr>
            <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">Single-step</td>
          ...
          <tr>
            <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">Multi-step</td>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          {([
            ...
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">{mode}</td>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          <tr>
            <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">At least X (greaterThanOrEqualTo)</td>
          ...
          <tr>
            <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">At most X (lessThanOrEqualTo)</td>
          ...
          <tr>
            <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">Exactly X (equalTo)</td>
      ...
      <p className="mb-3">Two AI calls happen simultaneously because they are independent of each other.</p>
      ...
      <p className="mb-3">The AI also checks inventory constraints: if there is only 1 collection and 0 product types, it returns single-step regardless of what the merchant typed.</p>
      ...
      <p className="mb-3"><strong>Example A</strong> goes through here: the Structure AI sees &quot;Buy 3 get 10% off, Buy 5 get 20% off&quot; -- no sequential flow -- and returns single-step.</p>
      <p className="mb-6"><strong>Example B</strong> goes through here: the Structure AI detects &quot;First pick... then pick...&quot; and returns multi-step with hints like &quot;meals&quot; → hot meals/cold meals collections, &quot;drinks&quot; → soft drinks/smoothies.</p>
      ...
      <p className="mb-6">The AI also handles locale-formatted numbers (e.g., &quot;149,50&quot; becomes &quot;149.50&quot;) and mixed-mode conflicts (when tiers mix different modes, only the majority mode is kept).</p>
      ...
      <p className="mb-3"><strong>Example A:</strong> Discount AI extracts Percentage mode with tiers 3→10% and 5→20%.</p>
      ...
      <p className="mb-3">This is a deterministic (non-AI) step that takes the Structure AI&apos;s output and the merchant&apos;s actual inventory to build the product categories for each step.</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-6">
        ...
        <tbody>
          {([
            ['Collections only', 'One category per collection (e.g., 3 collections = 3 categories)'],
            ['Products only (no custom groups)', 'One category per product type (products without a type go into "Other")'],
          ...
          ] as [string, string][]).map(([inv, strat], i) => (
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2">{inv}</td>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2">{strat}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <ul className="list-disc list-inside mb-6 space-y-2">
        ...
        <li>Steps that were auto-generated (not from the merchant&apos;s text) and have no real categories are removed.</li>
      </ul>

      <p className="mb-3"><strong>Example A:</strong> Two collections (Summer Sale, Winter Collection) become two categories in a single step: &quot;Summer Sale&quot; and &quot;Winter Collection&quot;.</p>
      ...
      <p className="mb-3">The Rules AI determines quantity constraints for each step. It receives the merchant text plus the category structure built in Phase 1.5 and the discount output from Phase 1.</p>
      <p className="mb-3"><strong>Critical exception: BXGY skips this entirely.</strong> When the Discount AI returns BXGY mode, the Rules AI is not called at all. The assembly layer handles BXGY rules deterministically (see Phase 2.5).</p>
      ...
      <p className="mb-6">The AI can also return <strong>per-category rules</strong> when the merchant names specific product groups with specific quantities (e.g., &quot;Select 2 Converse and 3 Nike&quot;). This only happens when: 1. The merchant explicitly names distinct product groups with per-group quantities, 2. Category context confirms those groups exist as categories, 3. The step has more than one category.</p>

      <h3 className="text-xl font-semibold mb-3">Phase 2.5: Deterministic Assembly</h3>
      <p className="mb-3">This is the final phase where all AI outputs are combined into a bundle configuration. There is no AI involved -- it is pure logic with fallbacks.</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-6">
        ...
        <tbody>
          {([
            ['P1', 'Discount mode is Fixed Bundle Price', 'Force "exactly X" on all steps. For multi-step, distribute total across steps (e.g., 5 across 2 steps = 3+2).', '"5 items for $99" → exactly 5 (or 3+2 for multi-step)'],
            ['P1.5', 'Discount mode is BXGY', 'Single-step: min qty = buyQty + getQty (minimum 2). Multi-step: Step 1 min = buyQty (minimum 2), Step 2 min = getQty (minimum 1, no floor of 2). Steps 3+ get default "at least 2".', '"Buy 2 get 1 free" → at least 3 (single) or Step 1 at least 2, Step 2 at least 1 (multi)'],
            ['P2', 'Rules AI returned explicit conditions', 'Use them directly. If per-category rules exist, apply to matching categories (case-insensitive exact match).', '"Select 2 Converse and 3 Nike" → per-category rules'],
            ['P3', 'No explicit rules, but discount tiers exist', 'Bridge the minimum quantity from the lowest discount tier. Multi-step: default each step to at least 1.', 'Discount says "Buy 3 → 10%", so minimum is at least 3'],
            ['P4', 'Nothing at all', 'Single-step: at least 2. Multi-step: at least 1 per step.', 'Bare minimum fallback'],
          ] as [string, string, string, string][]).map(([priority, condition, what, example], i) => (
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">{priority}</td>
      ...
      <p className="mb-3"><strong>Rule deduplication:</strong> Each step can have at most one &quot;at least&quot; rule and one &quot;at most&quot; rule. When duplicates exist: the lowest &quot;at least&quot; value is kept (safest minimum) and the highest &quot;at most&quot; value is kept (safest maximum). &quot;Exactly&quot; rules can have multiple values.</p>
      ...
      <p className="mb-3">Every pipeline run produces:</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          {([
            ...
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">{flag}</td>
      ...
      <p className="mb-8">The transformer layer converts the pipeline output into the correct format for whichever display type is selected. For BXGY bundles, the internal &quot;BXGY&quot; label is converted to &quot;BOGO&quot; for the frontend.</p>
      ...
      <ol className="list-decimal list-inside mb-8 space-y-2">
        ...
        <li><strong>No product title awareness in category building.</strong> Product group hints from the AI are filtered against real product titles (case-insensitive exact match). If the AI hallucinates a product title, it is silently removed.</li>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-6">
        ...
        <tbody>
          ...
          ] as [string, string, string][]).map(([error, retried, what], i) => (
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2">{error}</td>
      ...
      <p className="mb-3">When an individual AI layer fails, the pipeline continues with safe defaults for that layer. Only an unhandled exception in the pipeline runner itself causes an ERROR status.</p>
      <p className="mb-8"><strong>Error fallback structure:</strong> On total pipeline crash, the response is: empty steps, Percentage discount mode with default tiers (2→5%, 3→10%, 4→15%), status &quot;ERROR&quot;, empty flags, empty decision trace.</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-6">
        ...
        <tbody>
          {([
            ...
            <tr key={i}>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium"><code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">{tag}</code></td>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2">{meaning}</td>
            </tr>
          ))}
        </tbody>
      ...
      <p className="mb-3">Every pipeline run returns a <code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">decision_trace</code> object and a <code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">_decisionSummary</code> text.</p>
      ...
      <pre className="bg-slate-100 dark:bg-slate-800 p-4 rounded text-sm mb-6 overflow-x-auto">
{`Structure: The merchant describes two groups: meals and drinks, in sequence.
...
Rules: Per-step rules: 10 meals and 5 drinks.
  → EXPLICIT_CONDITIONS: rulesOutput has 2 condition(s)`}
      </pre>
        ...
        <tbody>
          {([
            ...
            <tr key={i}>
              ...
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2 font-medium">{pattern}</td>
              <td className="border border-slate-300 dark:border-slate-600 px-3 py-2">{meaning}</td>
            </tr>
          ))}
        </tbody>
      ...
      <p className="mb-3">The transformer converts the internal pipeline output into the frontend-ready payload. Key behaviors:</p>
      ...
      <p className="mb-3"><strong>Category ID generation:</strong> Each category gets a randomly generated 5-digit numeric ID (e.g., &quot;category47382&quot;). These IDs are generated at transform time and are not deterministic.</p>
      ...
      <ul className="list-disc list-inside mb-6 space-y-2">
        ...
        <li>Discount applies to &quot;lowest_priced&quot; item</li>
      ...
      <p className="mb-3"><strong>Discount text templates:</strong> The transformer generates pre-formatted discount messaging text for the storefront (e.g., &quot;Add 2 product(s) to save 10%!&quot;). These are templated with placeholders that the frontend fills in dynamically.</p>

      <p className="mb-8"><strong>Progress bar data:</strong> Generated from discount rules. Each tier gets a progress bar entry with title (e.g., &quot;3 Pack&quot;), subtitle (e.g., &quot;Save 10%&quot;), and a randomly generated 3-digit rule ID.</p>

      <hr className="border-slate-200 dark:border-slate-700 mb-8" />

      {/* ─── Database Logging ─── */}
      <h2 className="text-2xl font-semibold mb-4">Database Logging</h2>
      <p className="mb-3">Every pipeline run (unless <code className="text-sm bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5">skipLogging: true</code> is passed) is logged to a MongoDB collection with:</p>
      ...
      <p className="mb-3">The pipeline sends Slack notifications in the following situations:</p>
      ...
      <table className="w-full border border-slate-300 dark:border-slate-600 text-sm mb-8">
        ...
        <tbody>
          ...
          ] as [string, string][]).map(([situation, content], i) => (
            <tr key={i}>
